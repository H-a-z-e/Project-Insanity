...
//		gameFrame554 = in.readInt() == 1 ? true : false;
	...
	public void switchGameframe(int version) {
		if(version == 508) {
			...
			mapArea = new Sprite("508/maparea");
...
//	notifyItemSpawn(item, i + baseX, j + baseY);
	...
	public void drawSideIcons(){
		if(newMap) {
			...
			if(tabInterfaceIDs[9] != -1)//ignore
				sideIcons[9].drawSprite(79, 306);
			if(tabInterfaceIDs[10] != -1)//clan
				sideIcons[7].drawSprite(113, 302);
			if(tabInterfaceIDs[11] != -1)//options
				sideIcons[11].drawSprite(145, 304);
			if(tabInterfaceIDs[12] != -1)//emotes
				sideIcons[12].drawSprite(181, 302);
	...
	private void resetImageProducers()
	{
		...
		loginScreenArea = new RSImageProducer(765, 503, getGameComponent());
		DrawingArea.setAllPixelsToZero();
		aRSImageProducer_1109 = new RSImageProducer(360, 275, getGameComponent());
		DrawingArea.setAllPixelsToZero();
	...
	void drawLoadingText(int i, String s)
	{
		...
		aRSImageProducer_1109.initDrawingArea();
		loadingBarEmpty = new Sprite("GameFrame/OTHER/empty");
		loadingBarFull = new Sprite("GameFrame/OTHER/full");
		char c = '\u0168';
		char c1 = '\310';
		byte byte1 = 20;
		int j = c1 / 2 - 18 - byte1;
...
if (l == 713) {
	...
	private void processTabClick() {
		...
		if(super.clickMode3 == 1) {
			...
			if(newMap) {
				if(super.mouseX >= 628 && super.mouseX <= 659 && super.mouseY >= 466 && super.mouseY <= 503 && tabInterfaceIDs[10] != -1) {
					needDrawTabArea = true;
					tabID = 10;
					tabAreaAltered = true;
				}
 ...
 //		   return;
		...
		if(signlink.cache_dat != null)
		{
           ...
           titleStreamLoader = streamLoaderForName(1, "title screen", "title", expectedCRCs[1], 25);
			...
			loginBox = new Sprite("Gameframe/title");
			loginBoxHover = new Sprite("Gameframe/box_hover");
			loginHover = new Sprite("Gameframe/login_hover");
			loginHoverWorld = new Sprite("Gameframe/login_hover2");
		...
		xpFlag = new Sprite("Gameframe/xpFlag");
			...
			for(int i = 0; i <= 19; i++) {
				ORB[i] = new Sprite("Gameframe/ORB/Orb "+i+"");
			}
			for(int i = 1; i <= 15; i++) {
...
public int getOrbTextColor(int statusInt){
	...
	public void drawHPOrb() {
		int health;
		String OrbDirectory = signlink.findcachedir()+"/Sprites/Gameframe/ORB/";
	...
	public void drawRunOrb(){
		String OD = signlink.findcachedir()+"/Sprites/Gameframe/ORB/";
...
public int world = 1;
	...
	private void drawLoginScreen(boolean flag) {
		...
		int l = c / 2 - 80;
		int k1 = c1 / 2 + 20;
		aTextDrawingArea_1271.method382(0xffff00, 130 + x, "", // Input World
					k1 + 22
					, true);
		// Welcome screen
		if (loginScreenState == 0) {
			...
			if (screenHover[2]) {
				loginHover.drawSprite(l - 58 + x, k1 + 41);
			}
			if (screenHover[3]) {
				loginHoverWorld.drawSprite(l + 116 + x, k1 + 42);
			}
			aTextDrawingArea_1271.method382(0xffff00, 132 + x, loginMessage1,
					k1 + 41 + 40, true);
			aTextDrawingArea_1271.method382(0xffff00, 132 + x, loginMessage2,
					k1 + 41 + 55
					, true);
			...
			aTextDrawingArea_1271
					.method389(true, c / 2 - 154 + x, 0xffffff, ""
							+ myUsername
							+ ((loginScreenCursorPos == 0)
...
//private RSImageProducer loginScreenArea;
	...
	private void processLoginScreenInput() {
		if(loginMessage2.equals("@whi@Error connecting to server.") && isLoggingIn){
			isLoggingIn = false;
		}
		if (loginScreenState == 0) {
				...
				screenHover[2] = super.mouseX >= 293 && super.mouseX <= 471 && super.mouseY >= 296 + yoffset && super.mouseY <= 321 + yoffset;
				//screenHover[3] = super.mouseX >= 465 && super.mouseX <= 492 && super.mouseY >= 296 + yoffset && super.mouseY <= 321 + yoffset;
