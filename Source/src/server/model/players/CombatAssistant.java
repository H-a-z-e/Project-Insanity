...
public class CombatAssistant{
	...
	**/
	...
	public void playerDelayedHit(int i) {
		if (Server.playerHandler.players[i] != null) {
			...
			if(!c.castingMagic && c.projectileStage > 0) { // range hit damage
				...
				applySmite(i, damage);
				if (c.eScapeDelay <= 1) {
                    applyeScape(i, damage);//makes it like smite
                }
				if (damage2 != -1)
	...
	public void applynpceScape(int index, int damage) {
                if (!c.curseActive[18])
                        return;
					if (c.oldNpcIndex > 0) {
            ...
            public void applyeScape(int index, int damage) {
                if (!c.curseActive[18])
                        return;
                if (Server.playerHandler.players[index] != null) { 
