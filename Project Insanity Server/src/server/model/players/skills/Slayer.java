...
public class Slayer {
	...
	public Slayer(final Client c) {
		this.c = c;
	}

	public boolean canDoTask(final int taskLevel, final int random) {
		if (taskLevel == 1) {
	...
	public void giveTask(final int taskLevel) {
		int given = 0;
		int random = 0;
		if (taskLevel == 1) {
		...
		c.sendMessage("You have been assigned to kill " + c.taskAmount + " " + NPCHandler.getNpcListName(given) + " as a slayer task.");
	}

	public void giveTask2() {
		for (final int lowTask : lowTasks) {
			if (lowTask == c.slayerTask) {
				c.sendMessage("You already have an easy task... to kill " + c.taskAmount + " " + NPCHandler.getNpcListName(c.slayerTask) + ".");
				return;
			}
		}
		giveTask(1);
