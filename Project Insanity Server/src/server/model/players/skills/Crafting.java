...
public class Crafting {
	...
	public Crafting(final Client c) {
		this.c = c;
	}
	...
	public void cutGem(final int id) {
		for (final int[] gem : gems) {
			if (gem[0] == id) {
				if (c.playerLevel[c.playerCrafting] >= gem[2]) {
					c.getItems().deleteItem(id, c.getItems().getItemSlot(id),1);
					c.getItems().addItem(gem[1], 1);
					c.getPA().addSkillXP(gem[3] * Config.CRAFTING_EXPERIENCE, c.playerCrafting);
					break;
				}
			}
		}
	...
	public void handleChisel(final int id1, final int id2) {
		if (id1 == 1755) {
			cutGem(id2);
		} else {
			cutGem(id1);
	...
	public void handleCraftingClick(final int clickId) {
	...
	public void handleLeather(final int item1, final int item2) {
		if (item1 == 1733) {
			openLeather(item2);
		} else {
			openLeather(item1);
	...
	public void openLeather(final int item) {
		if (item == 1745) {
