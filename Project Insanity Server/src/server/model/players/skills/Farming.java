...
public class Farming {
	...
	public Farming(final Client c) {
		this.c = c;
	}

	public void checkItemOnObject(final int itemId) {
	...
	private void handleFarming(final int seedId, final int herbId, final int exp, final int slot) {
		...
		if (c.getItems().playerHasItem(seedId,1)) {
			...
			final int herbAmount = Misc.random(5) + 3;
			c.farm[0] = herbId;
			c.farm[1] = herbAmount;
			updateHerbPatch();
		}
