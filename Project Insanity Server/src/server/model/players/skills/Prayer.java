...
public class Prayer {
	...
	public static void buryBones(final Client c, final int i, final int slot) {
		Prayer.handleBones(c, i, false, slot);
	}

	private static void handleBones(final Client c, final int i, final boolean altar, final int slot) {
		for (final int[] element : Prayer.BONES) {
			if(i == element[0]) {
				if(System.currentTimeMillis() - c.buryDelay > 800) {
					c.getItems().deleteItem(element[0], slot, 1);
					c.getPA().addSkillXP((altar ? element[1] * 2 : element[1]) * Prayer.PRAYER_XP, 5);
	...
	public static void useBonesOnAltar(final Client c, final int l, final int i) {
		Prayer.handleBones(c, i, true, c.getItems().getItemSlot(i));
	}
