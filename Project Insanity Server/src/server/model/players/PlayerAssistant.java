...
public class PlayerAssistant{
	...
	public PlayerAssistant(final Client Client) {
		c = Client;
	}
	...
	public boolean checkForFlags() {
		final int[][] itemsToCheck = {{995,100000000},{35,5},{667,5},{2402,5},{746,5},{4151,150},{565,100000},{560,100000},{555,300000},{11235,10}};
	...
	public void checkObjectSpawn(final int objectId, final int objectX, final int objectY, final int face, final int objectType) {
	...
	public void createObjectHints(final int x, final int y, final int height, final int pos) {
		synchronized(c) {
			if(c.getOutStream() != null && c != null) {
	...
	public void createPlayersProjectile(final int x, final int y, final int offX, final int offY, final int angle, final int speed, final int gfxMoving, final int startHeight, final int endHeight, final int lockon, final int time) {
		synchronized(c) {
	...
	public void createPlayersProjectile2(final int x, final int y, final int offX, final int offY, final int angle, final int speed, final int gfxMoving, final int startHeight, final int endHeight, final int lockon, final int time, final int slope) {
		synchronized(c) {
			for(int i = 0; i < Config.MAX_PLAYERS; i++) {
				final Player p = PlayerHandler.players[i];
				if(p != null) {
	...
	public void createPlayersStillGfx(final int id, final int x, final int y, final int height, final int time) {
		synchronized(c) {
			for(int i = 0; i < Config.MAX_PLAYERS; i++) {
				final Player p = PlayerHandler.players[i];
				if(p != null) {
	 ...
	 * Creating projectile
	 **/
	public void createProjectile(final int x, final int y, final int offX, final int offY, final int angle, final int speed, final int gfxMoving, final int startHeight, final int endHeight, final int lockon, final int time) {
		synchronized(c) {
			if(c.getOutStream() != null && c != null) {
				c.getOutStream().createFrame(85);
	...
	public void createProjectile2(final int x, final int y, final int offX, final int offY, final int angle, final int speed, final int gfxMoving, final int startHeight, final int endHeight, final int lockon, final int time, final int slope) {
		synchronized(c) {
			if(c.getOutStream() != null && c != null) {
				c.getOutStream().createFrame(85);
				c.getOutStream().writeByteC(y - c.getMapRegionY() * 8 - 2);
	...
	public void followNpc() {
		...
		final int otherX = NPCHandler.npcs[c.followId2].getX();
		final int otherY = NPCHandler.npcs[c.followId2].getY();
		final boolean withinDistance = c.goodDistance(otherX, otherY, c.getX(), c.getY(), 2);
		c.goodDistance(otherX, otherY, c.getX(), c.getY(), 1);
		final boolean hallyDistance = c.goodDistance(otherX, otherY, c.getX(), c.getY(), 2);
		final boolean bowDistance = c.goodDistance(otherX, otherY, c.getX(), c.getY(), 8);
		final boolean rangeWeaponDistance = c.goodDistance(otherX, otherY, c.getX(), c.getY(), 4);
	...
	public void followPlayer() {
		...
		final boolean hallyDistance = c.goodDistance(otherX, otherY, c.getX(), c.getY(), 2);
		final boolean rangeWeaponDistance = c.goodDistance(otherX, otherY, c.getX(), c.getY(), 4);
		final boolean bowDistance = c.goodDistance(otherX, otherY, c.getX(), c.getY(), 6);
		final boolean mageDistance = c.goodDistance(otherX, otherY, c.getX(), c.getY(), 7);
		final boolean castingMagic = (c.usingMagic || c.mageFollow || c.autocasting || c.spellId > 0) && mageDistance;
		final boolean playerRanging = c.usingRangeWeapon && rangeWeaponDistance;
		final boolean playerBowOrCross = c.usingBow && bowDistance;
		if (!c.goodDistance(otherX, otherY, c.getX(), c.getY(), 25)) {
			c.followId = 0;
			resetFollow();
			return;
	...
	public void giveLife() {
		...
		if(c.duelStatus <= 4 && !c.getPA().inPitsWait()) { // if we are not in a duel we must be in wildy so remove items
			if (!c.inPits && !c.inFightCaves()) {
				...
				if(c.playerRights == 0 && Config.REGULAR_DROP_ITEMS || c.playerRights != 0) {
					c.sendMessage("@red@Dropping items as a Regular Player has been disabled.");
				} else if(c.playerRights == 1 && Config.DONATOR_DROP_ITEMS || c.playerRights != 1) {
					c.sendMessage("@red@Dropping items as a Donator has been disabled.");
				} else if(c.playerRights == 2 && Config.SUPDONATOR_DROP_ITEMS || c.playerRights != 2) {
					c.sendMessage("@red@Dropping items as a Donator has been disabled.");
				} else if(c.playerRights == 3 && Config.MODERATOR_DROP_ITEMS || c.playerRights != 3) {
					c.sendMessage("@red@Dropping items as a Donator has been disabled.");
				} else if(c.playerRights == 4 && Config.DEVELOPER_DROP_ITEMS || c.playerRights != 4) {
					c.sendMessage("@red@Dropping items as a Donator has been disabled.");
				} else if(c.playerRights == 5 && Config.COOWNER_DROP_ITEMS || c.playerRights != 5) {
					c.sendMessage("@red@Dropping items as a Donator has been disabled.");
				} else if(c.playerRights == 6 && Config.OWNER_DROP_ITEMS || c.playerRights != 6) {
					c.sendMessage("@red@Dropping items as a Donator has been disabled.");
			...
			if(friend != 0)  {
				for(int i2 = 1; i2 < Config.MAX_PLAYERS; i2++) {
					...
					if (p != null && p.isActive && Misc.playerNameToInt64(p.playerName) == friend)  {
						final Client o = (Client)p;
						if(o != null) {
							if (c.playerRights >= 3 || p.privateChat == 0 || p.privateChat == 1 && o.getPA().isInPM(Misc.playerNameToInt64(c.playerName))) {
								loadPM(friend, 1);
								pmLoaded = true;
							}
							break;
	...
	public void magicOnItems(final int slot, final int itemId, final int spellId) {
		...
		case 1162: // low alch
			if(System.currentTimeMillis() - c.alchDelay > 1000) {
				...
				final int reward = c.getShops().getItemShopValue(itemId/3);
				final int playerAmount = c.getItems().getItemAmount(995);
				if(reward + playerAmount < -1) {
		...
		case 1178: // high alch
			if(System.currentTimeMillis() - c.alchDelay > 2000) {
				...
				final int reward = (int)(c.getShops().getItemShopValue(itemId)*.75);
				final int playerAmount = c.getItems().getItemAmount(995);
				if(reward + playerAmount < -1) {
					c.sendMessage("You can't alch anymore!");
					break;
	...
	public void moveCheck(final int xMove, final int yMove) {
		movePlayer(c.absX + xMove, c.absY + yMove, Player.heightLevel);
	}
	 ...
	 **/
	public void object(final int objectId, final int objectX, final int objectY, final int face, final int objectType) {
		synchronized(c) {
			if(c.getOutStream() != null && c != null) {
				c.getOutStream().createFrame(85);
				c.getOutStream().writeByteC(objectY - c.getMapRegionY() * 8);
	...
	public void playerWalk(final int x, final int y) {	//Checks Player Walk Location
	 ...
	 */
	public void potionPoisonHeal(final int itemId, final int itemSlot, final int newItemId, final int healType) {
	...
	public void processTeleport() {
		...
		Player.heightLevel = c.teleHeight;
	...
	public int randomBarrows() {
		return PlayerAssistant.Barrows[(int)(Math.random()*PlayerAssistant.Barrows.length)];
	}
	...
	public int randomRunes() {
		return PlayerAssistant.Runes[(int) (Math.random()*PlayerAssistant.Runes.length)];
	}

	public void refreshSkill(final int i) {
		switch (i) {
		case 0:
	...
	public void sendFrame164(final int Frame) {
		synchronized(c) {
			if(c.getOutStream() != null && c != null) {
	...
	public void sendFrame246(final int MainFrame, final int SubFrame, final int SubFrame2) {
		synchronized(c) {
			if(c.getOutStream() != null && c != null) {
	...
	public void sendFrame34(final int id, final int slot, final int column, final int amount) {
		synchronized(c) {
			if(c.getOutStream() != null && c != null) {
	...
	public void sendFrame75(final int MainFrame, final int SubFrame) {
		synchronized(c) {
			if(c.getOutStream() != null && c != null) {
	...
	public void sendFrame87(final int id, final int state) {
		synchronized(c) {
			if(c.getOutStream() != null && c != null) {
	...
	public void sendFrame99(final int state) { // used for disabling map
		synchronized(c) {
			if(c.getOutStream() != null && c != null) {
				if(mapStatus != state) {
					mapStatus = state;
	...
	public void sendPM(final long name, final int rights, final byte[] chatmessage, final int messagesize) {
		synchronized(c) {
			if(c.getOutStream() != null && c != null) {
	...
	public void setChatOptions(final int publicChat, final int privateChat, final int tradeBlock) {
		synchronized(c) {
			if(c.getOutStream() != null && c != null) {
	...
	public void setPrivateMessaging(final int i) { // friends and ignore list status
		synchronized(c) {
			if(c.getOutStream() != null && c != null) {
	...
	public void setSkillLevel(final int skillNum, final int currentLevel, final int XP) {
		synchronized(c) {
			if(c.getOutStream() != null && c != null) {
	...
	public void startTeleport(final int x, final int y, final int height, final String teleportType) {
		if(c.duelStatus == 5) {
	 ...
	 * GFX
	 */
	public void stillGfx(final int id, final int x, final int y, final int height, final int time) {
		synchronized(c) {
			if(c.getOutStream() != null && c != null) {
				c.getOutStream().createFrame(85);
	...
	public void updatePM(final int pID, final int world) { // used for private chat updates
		...
		final long l = Misc.playerNameToInt64(PlayerHandler.players[pID].playerName);
	...
	public void useCharge() {
		final int[][] glory = {{1712, 1710, 3}, {1710, 1708, 2}, {1708, 1706, 1}, {1706, 1704, 1}};
		for (final int[] element : glory) {
			if (c.itemUsing == element[0]) {
				...
				if (element[2] > 1) {
					c.sendMessage("Your amulet has "+element[2]+" charges left.");
				} else {
					c.sendMessage("Your amulet has "+element[2]+" charge left.");
				}
			}
		}
	...
	public void useOperate(final int itemId) {
		switch (itemId) {
